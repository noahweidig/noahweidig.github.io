...
# without an R toolchain in CI. Instead the quarto-webr extension turns some
# blocks into live cells that run in the reader's browser via WebAssembly
# (webR). Blocks a browser sandbox cannot honestly run (renv, real files)
# stay as static listings.
#
# Cells are fenced as ```{.r .webr}, not ```{webr-r}: code-window filters
# rewrite unknown code-block languages and would strip the class webR keys on.
#
# `engine: markdown` keeps Quarto from reaching for jupyter or knitr.
engine: markdown
webr:
  packages: []
...
Cells with a **Run Code** button run real R, compiled to WebAssembly via
[webR](https://docs.r-wasm.org/webr/latest/) and
[quarto-webr](https://github.com/coatless/quarto-webr) — nothing installed,
nothing sent anywhere. Run the setup cell first; later cells share its
session. `webr::install()` is a browser-only line — on your own machine
just use `install.packages()` once.
:::
...
Implements the *grammar of graphics*: data mapped to visual properties
...
*guarantees* a double vector back, erroring loudly otherwise. [More below](#purrr-in-depth).
...
(`renv.lock`) recording exact versions — a virtualenv/lockfile for R. Core
...
- **parameters** turns coefficients, SEs, CIs, and *p*-values into one
...
- **effectsize** computes and interprets standardized effect sizes
  (Cohen's *d*, eta-squared) in plain language.
...
```{.r .webr}
webr::install(c("parameters", "report"))
